...
public class Excel2HtmlTEngineController extends AbstractTransformerController {
  ...
  public void transformImpl(String transformName, String sourceMimetype, String targetMimetype, Map<String, String> transformOptions, File sourceFile, File targetFile) {
    ...
    File targetTemp = null;
    try {
      String sourceFileName = sourceFile.getName();
      String targetName = targetFile.getName();
      String sourceExtension = StringUtils.getFilenameExtension(sourceFileName);
      String targetExtension = StringUtils.getFilenameExtension(targetName);

      sourceTemp = FileManager.TempFileProvider.createTempFile("tempSource_", "." + sourceExtension);
      IOUtils.copy(new FileInputStream(sourceFile), new FileOutputStream(sourceTemp));
      targetTemp = FileManager.TempFileProvider.createTempFile("tempTarget_", "." + targetExtension);

      conversionEngine.transformExcelToHtml(sourceTemp, targetTemp);
      IOUtils.copy(new FileInputStream(targetTemp), new FileOutputStream(targetFile));
    } catch (Exception e) {
      ...
      if (sourceTemp != null && sourceTemp.exists()) {
        sourceTemp.delete();
      }
      if (targetTemp != null && targetTemp.exists()) {
        targetTemp.delete();
      }
    }
